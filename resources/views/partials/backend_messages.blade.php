...
@if (session('status_good'))
    <div align="center" class="alert alert-success " id="backend-msg" style="z-index: 1000; background-color: #dff0d8;">
        <p style="color: rebeccapurple;">{!! session('status_good') !!}</p>
    </div>
@endif
@if (session('status_bad'))
    <div align="center" class="alert alert-danger" id="backend-msg">
        {{ session('status_bad') }}
    </div>
@endif
